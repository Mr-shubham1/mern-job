...
export const login = async (req, res) => {
  try {
    ...
    return res.status(200).cookie('token', token, { 
      ...
      sameSite: 'none',
      secure: true               // Ensure sameSite is passed as a string
    }).json({
...
export const updateprofilepic = async (req,res)=>{
  try {
        ...
        if(user){
          if(optimizedUrl){
            user.profile.profilephoto = optimizedUrl;
            await user.save();
          }
        }
        ...
        if(!user){
          return res.status(400).json({
            success:false,
            message:"you must be logged in to update the profile pic"
          })
        }
    ...
    console.log("catch error in update profile pic controller");
  }
}      // ✅ tested